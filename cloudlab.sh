...
PRIVATE_IPv4="$(printf ${route#*src })";
export DNS_SERVER_1="${DNS_SERVER_1:-$PRIVATE_IPv4}";
...
###############
[[ "$INSTALL_WEBMIN" == "true" ]] && bash -c "$(curl -sSL https://github.com/asamahy/oci-openvpn/raw/dev/webmin.sh)"
[[ "$INSTALL_OPENVPN" == "true" ]] && bash -c "$(curl -sSL https://github.com/asamahy/oci-openvpn/raw/dev/openvpn.sh)"
[[ "$INSTALL_PIHOLE" == "true" ]] && bash -c "$(curl -sSL https://github.com/asamahy/oci-openvpn/raw/dev/pihole.sh)"
[[ "$INSTALL_CLOUDFLARED" == "true" ]] && bash -c "$(curl -sSL https://github.com/asamahy/oci-openvpn/raw/dev/cloudflared.sh)"
[[ "$INSTALL_UNBOUND" == "true" ]] && bash -c "$(curl -sSL https://github.com/asamahy/oci-openvpn/raw/dev/unbound.sh)"
[[ "$INSTALL_TAILSCALE" == "true" ]] && bash -c "$(curl -sSL https://github.com/asamahy/oci-openvpn/raw/dev/tailscale.sh)"
###############
...
printf "%s\n" "Part 6: Saving Log and Rebooting Instance in 5 seconds"
cp /var/log/cloud-init-output.log /root/cloud-init-output.log
sleep 5 && reboot